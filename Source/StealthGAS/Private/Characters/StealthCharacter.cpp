...
#include "Public/Characters/StealthCharacter.h"
...
void AStealthCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent)) {
		...
		EnhancedInputComponent->BindAction(MouseLookAction, ETriggerEvent::Triggered, this, &AStealthCharacter::Look);
		...
		// Abilities
		EnhancedInputComponent->BindAction(PrimaryAction, ETriggerEvent::Triggered, this, &ThisClass::DoPrimaryStart);
	...
	{
		UE_LOG(LogStealthGAS, Error, TEXT("'%s' Failed to find an Enhanced Input component! This template is built to use the Enhanced Input system. If you intend to use the legacy system, then you will need to update this C++ file."), *GetNameSafe(this));
	}
}
...
void AStealthCharacter::DoLook(float Yaw, float Pitch)
{
	if (GetController() != nullptr)
	{
...
void AStealthCharacter::DoCrouchStart()
{
	Crouch();
}

void AStealthCharacter::DoCrouchEnd()
{
	UnCrouch();
}
